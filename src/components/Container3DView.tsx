...
function ContainerWireframe({ container }: { container: ContainerType }) {
  ...
  return (
    <group>
      ...
      <Text position={[cW/2, -0.15, 0]} fontSize={0.15} color="#4a90d9" anchorX="center" anchorY="top">
        ← DOOR / TÜR →
      </Text>
    </group>
  );
...
export function Container3DView({ container, items, lang, activeItemId, onItemClick }: Props) {
  ...
  // Stable target based on container dimensions
  ...
  // Reset step count when items change
  useEffect(() => {
    setVisibleStep(items.length);
    stopPlay();
  }, [items.length, stopPlay]);

  // Camera preset handler — sets position directly (no animation loop conflicts)
  const applyPreset = useCallback((preset: string) => {
    ...
    switch (preset) {
      case 'iso':
        cam.position.set(8, 6, -4);
        break;
      case 'top':
        cam.position.set(target.x, 10, target.z + 0.01);
        break;
      case 'side':
        cam.position.set(-8, 2, target.z);
        break;
      case 'front':
        cam.position.set(target.x, 1.5, -6);
        break;
    }
